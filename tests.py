...
def _is_equal(x, y):
    ...
    if isinstance(x, pd.DataFrame) or isinstance(y, pd.Series):
        ...
        if len(x) == len(y):
            for x_item, y_item in zip(x, y, strict=False):
...
def generate_random_tickers(n_tickers=None):
    ...
    for ticker_symbol_rand, ticker_symbol_length in zip(ticker_symbol_random, ticker_symbol_lengths, strict=False):
...
def assert_output(fn, fn_inputs, fn_expected_outputs):
    assert isinstance(fn_expected_outputs, OrderedDict)
    ...
    if len(fn_expected_outputs) == 1:
        fn_outputs[list(fn_expected_outputs)[0]] = fn_raw_out  # noqa: RUF015
    elif len(fn_expected_outputs) > 1:
        assert isinstance(fn_raw_out, tuple),\
    ...
    for fn_out, (out_name, expected_out) in zip(fn_outputs.values(), fn_expected_outputs.items(), strict=False):
        ...
        if isinstance(expected_out, pd.DataFrame):
            ...
            # Sort Columns
            fn_out = fn_out.sort_index(axis=1)
            expected_out = expected_out.sort_index(axis=1)
